...
IEnumerator StopParticleAfterTime(float time)
{
   ...
   {  
     ...
     {  
         Vector3 directionToTriggerCenter =(transform.position - playerTransform.position).normalized; 
        
         bool isOnRightSideOfTriggerCenter =(Vector3.Dot(directionToTriggerCenter, transform.right) > 0); 
    
        
         if (!isPlayerInside && !isOnRightSideOfTriggerCenter) 
         {     
             raft.SetActive(false);  
             PlayEffects();  
             ResetPosition();    
